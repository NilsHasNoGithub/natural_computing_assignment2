...
def main(data_file: str, result_dir: str):
    ...
    k_means_results = [
        test_k_means(artificial_data, n_clusters) for _ in range(n_rounds)
    ]
    k_means_error = np.mean([s[1] for s in k_means_results])
    ...
    print(
        f"Quantization error POC for artificial data, averaged over {n_rounds} runs: {np.mean(scores)}"
    )
